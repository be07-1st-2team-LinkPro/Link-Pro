...
begin
    ...
    if 점수>5 or 점수<1 then
        ...
        if (select count(*) from client_review where review_order_id = 주문번호 and review_client_id = 구매자ID) =1 then
            select '이미 리뷰를 작성한 주문입니다.';
        else
        -- order state가 완료일 때만 리뷰 작성이 가능하다
            if (select order_state from order_service where order_pro_id = 개발자ID and order_id = 주문번호)='done' then
